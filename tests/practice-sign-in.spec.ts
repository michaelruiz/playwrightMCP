...
test.describe('Signal Lab practice page', { tag: ['@forms', '@regression'] }, () => {
  test(
    'sign-in shows a success message for the configured login plan',
    { tag: '@smoke' },
    async ({ practicePage, loginCredentials, reportStep, signInWithConfiguredCredentials }) => {
      await reportStep('Open the practice page', async () => {
        await expect(practicePage.signInStatus).toHaveText('Waiting for sign-in input.');
      });
