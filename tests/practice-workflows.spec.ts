...
test.describe('Signal Lab practice workflows', { tag: ['@ui', '@regression'] }, () => {
  test(
    'filtering cards narrows the results and updates the visible count',
    { tag: '@smoke' },
    async ({ practicePage, reportStep }) => {
  ...
  test(
    'adding a task updates the task count and task list',
    { tag: '@smoke' },
    async ({ practicePage, reportStep }) => {
  ...
  test('release notes opens as a dialog and can be dismissed', async ({
    ...
    await reportStep('Verify the dialog contents', async () => {
      ...
      await expect(
        practicePage.releaseDialogText('Task creation updates the board without a page refresh.'),
      ).toBeVisible();
    });
